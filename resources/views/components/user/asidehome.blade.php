<aside id="sidebar-default" class="mr-12 relative hidden left-0 flex lg:flex flex-shrink-0 flex-col w-64 transition-width duration-75" aria-label="Sidebar">
    <div class="py-3 bg-red-500 text-white flex items-center text-base">
        <svg class="w-6 h-6 ml-8 mr-3" aria-hidden="true" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" d="M3 5a1 1 0 011-1h12a1 1 0 110 2H4a1 1 0 01-1-1zM3 10a1 1 0 011-1h12a1 1 0 110 2H4a1 1 0 01-1-1zM3 15a1 1 0 011-1h12a1 1 0 110 2H4a1 1 0 01-1-1z" clip-rule="evenodd"></path></svg>
        Semua Kategori
    </div>
    <div class="relative flex-1 flex flex-col min-h-0 border-gray-200 pt-4" style="background-color: #ffebeb">
        <div class="pl-8 flex-1 flex flex-col overflow-y-auto">
            <div class="flex-1 divide-y space-y-1">
            <ul class="space-y-2 pb-2">
                @foreach($categories as $category)
                    <li>
                        <a href="{{ route('user.books.search', ['category' => $category->id, 'based_on' => request()->query('based_on')]) }}" class="text-base text-gray-900 font-normal rounded-lg flex items-center py-2 hover:bg-gray-100 group">
